...
LOGGER = logging.getLogger("app.shadow_audit.multimodal_runner_bridge")

RUNNER_PYTHON_PATH = Path(settings.cap_cop_clinical_mlp_runner_python_path)
RUNNER_SCRIPT_PATH = Path("/srv/medorion/app/model-runners/cap_cop_multimodal_resnet18_runner.py")
RUNNER_TIMEOUT_SECONDS = max(int(settings.cap_cop_clinical_mlp_shadow_timeout_seconds), 300)
...
def invoke_multimodal_runner(payload: dict[str, Any] | None = None, *, check_artifact: bool = False) -> RunnerInvocationResult:
    ...
    if missing is not None:
        ...
        LOGGER.warning("Multimodal runner unavailable: %s", error_message)
    ...
    try:
        ...
        LOGGER.warning("Multimodal runner timed out after %s seconds", RUNNER_TIMEOUT_SECONDS)
        return RunnerInvocationResult(
            command=command,
            exit_code=None,
            stdout="",
        ...
        LOGGER.warning("Multimodal runner unavailable: %s", exc)
        return RunnerInvocationResult(
            command=command,
            exit_code=None,
            stdout="",
        ...
        LOGGER.exception("Unexpected failure invoking multimodal runner")
        return RunnerInvocationResult(
            command=command,
            exit_code=None,
            stdout="",
